...
TracerRegisters::TracerRegisters(uint8_t MAX485_RE_NEG, uint8_t MAX485_DE, Stream& serial) {
    ...
    this->node.setResponseTimeout(1000);
    this->node.begin(1, serial);
}

char* TracerRegisters::getRegistersValueInJson() {
    ...
    jsonString += "\"BATTERY_STATUS_TEXT\":" + to_string(this->variableValues[SolarTracerVariables::BATTERY_STATUS_TEXT]) + ",";
    jsonString += "\"CHARGING_EQUIPMENT_STATUS_TEXT\":" + to_string(this->variableValues[SolarTracerVariables::CHARGING_EQUIPMENT_STATUS_TEXT]) + ",";
    jsonString += "\"DISCHARGING_EQUIPMENT_STATUS_TEXT\":" + to_string(this->variableValues[SolarTracerVariables::DISCHARGING_EQUIPMENT_STATUS_TEXT]) + ",";
...
void TracerRegisters::AddressRegistry_3100() {
    uint8_t result = this->node.readInputRegisters(TracerRegisterType::MODBUS_ADDRESS_PV_VOLTAGE, 16);
...
void TracerRegisters::AddressRegistry_331B() {
    uint8_t result = this->node.readInputRegisters(TracerRegisterType::MODBUS_ADDRESS_BATTERY_OVERALL_CURRENT, 16);
    if (result != this->node.ku8MBSuccess) {
        return;
    }
...
void TracerRegisters::AddressRegistry_9003() {
    uint8_t result = this->node.readInputRegisters(TracerRegisterType::MODBUS_ADDRESS_HIGH_VOLTAGE_DISCONNECT, 12);
    if (result != this->node.ku8MBSuccess) {
        return;
    }
...
void TracerRegisters::updateStats() {
    uint8_t result = this->node.readInputRegisters(TracerRegisterType::MODBUS_ADDRESS_STAT_MAX_PV_VOLTAGE_TODAY, 12);
    if (result != this->node.ku8MBSuccess) {
        return;
    }
